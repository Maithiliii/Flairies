...
const RentScreen = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      console.log('Rent Screen: Fetching items from:', `${API_URL}/api/items/?listing_type=rent`);
      const response = await fetch(`${API_URL}/api/items/?listing_type=rent`);
      const data = await response.json();
      
      console.log('Rent Screen: Response status:', response.status);
      console.log('Rent Screen: Items received:', data.length);
      ...
      if (response.ok) {
        ...
        console.error('Rent Screen: Failed to fetch items:', data);
      }
    } catch (error) {
      console.error("Rent Screen: Network error:", error);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.heading}>Borrow the moment</Text>
      <Text style={styles.subheading}>Luxury looks for the weekend, straight from the community.</Text>
      
      {loading ? (
        <ActivityIndicator size="large" color="#044861" style={{ marginTop: 40 }} />
      ) : items.length === 0 ? (
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Rental hub coming soon</Text>
          ...
          {items.map((item) => (
            <TouchableOpacity key={item.id} style={styles.itemCard} onPress={() => handleItemPress(item)} activeOpacity={0.7}>
              ...
              <View style={styles.itemInfo}>
                ...
                <Text style={styles.itemPrice}>₹{item.rent_price} / day</Text>
